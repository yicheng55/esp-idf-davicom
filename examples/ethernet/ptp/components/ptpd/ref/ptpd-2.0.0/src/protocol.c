...
void toState(PtpClock *ptpClock, UInteger8 state)
{
    ...
    case PTP_LISTENING:
        ...
        /* In P2P mode, continue peer delay measurement even in LISTENING state
         * per IEEE 802.1AS requirement */
        if (ptpClock->portDS.delayMechanism == P2P)
        {
            timerStart(PDELAYREQ_INTERVAL_TIMER, safeGetRand(pow2ms(ptpClock->portDS.logMinPdelayReqInterval + 1)),
                       ptpClock->itimer);
        }
    ...
    case PTP_UNCALIBRATED:
        ...
        case E2E:
            ...
            // ptpClock->itimer);
            timerStart(DELAYREQ_INTERVAL_TIMER, safeGetRand(pow2ms(ptpClock->portDS.logMinDelayReqInterval + 1)),
                       ptpClock->itimer);
            break;
        case P2P:
            // timerStart(PDELAYREQ_INTERVAL_TIMER, getRand(pow2ms(ptpClock->portDS.logMinPdelayReqInterval + 1)),
...
void doState(PtpClock *ptpClock)
{
    ...
    case PTP_PASSIVE:
        ...
        /* In P2P mode, all states should continue peer delay measurement */
...
static void handleAnnounce(PtpClock *ptpClock, Boolean isFromSelf)
{
    ...
    case PTP_SLAVE:
        ...
        isFromCurrentParent =
            isSamePortIdentity(&ptpClock->parentDS.parentPortIdentity, &ptpClock->msgTmpHeader.sourcePortIdentity);
...
static void handleSync(PtpClock *ptpClock, TimeInternal *time, Boolean isFromSelf)
{
    ...
    case PTP_SLAVE:
        ...
        if (getFlag(ptpClock->msgTmpHeader.flagField[0], FLAG0_TWO_STEP))
        {
            ...
            DBG_PRINTF("Waiting for FollowUp (TWO_STEP mode)\n");
        }
        else
        {
            ...
            // 添加T2時間戳調試信息
            DBG_PRINTF("T2 (SYNC receive timestamp): %ds %dns\n", ptpClock->timestamp_syncRecieve.seconds,
                    ptpClock->timestamp_syncRecieve.nanoseconds);

            // 添加T1時間戳調試信息
            DBG_PRINTF("T1 (SYNC origin  timestamp): %ds %dns\n", originTimestamp.seconds, originTimestamp.nanoseconds);
            DBG_PRINTF("Correction field: %ds %dns\n", correctionField.seconds, correctionField.nanoseconds);
...
static void handleFollowUp(PtpClock *ptpClock, Boolean isFromSelf)
{
    ...
    case PTP_SLAVE:
        ...
        // 添加FollowUp時間戳調試信息
        DBG_PRINTF("T1 (FollowUp precise origin timestamp): %ds %dns\n", preciseOriginTimestamp.seconds,
                   preciseOriginTimestamp.nanoseconds);
        DBG_PRINTF("T2 (from previous SYNC): %ds %dns\n", ptpClock->timestamp_syncRecieve.seconds,
                   ptpClock->timestamp_syncRecieve.nanoseconds);
        DBG_PRINTF("Total correction field: %ds %dns\n", correctionField.seconds, correctionField.nanoseconds);
...
static void handleDelayReq(PtpClock *ptpClock, TimeInternal *time, Boolean isFromSelf)
{
    ...
    case E2E:
        ...
        case PTP_SLAVE:
            ...
            //                /*Add latency*/
            //                addTime(&ptpClock->delay_req_send_time, &ptpClock->delay_req_send_time,
            //                &rtOpts->outboundLatency); break;
            //            }
            break;
...
static void handleDelayResp(PtpClock *ptpClock, Boolean isFromSelf)
{
    ...
    case E2E:
        ...
        case PTP_SLAVE:
            ...
            isCurrentRequest =
                isSamePortIdentity(&ptpClock->portDS.portIdentity, &ptpClock->msgTmp.resp.requestingPortIdentity);

            if (((ptpClock->sentDelayReqSequenceId - 1) == ptpClock->msgTmpHeader.sequenceId) && isCurrentRequest &&
                isFromCurrentParent)
            {
                /* TODO: revisit 11.3 */
                ...
                scaledNanosecondsToInternalTime(&ptpClock->msgTmpHeader.correctionfield, &correctionField);
                updateDelay(ptpClock, &ptpClock->timestamp_delayReqSend, &ptpClock->timestamp_delayReqRecieve,
                            &correctionField);
...
static void handlePDelayReq(PtpClock *ptpClock, TimeInternal *time, Boolean isFromSelf)
{
    ...
    case P2P:
        ...
        case PTP_MASTER:
        {
            ...
            /* According to IEEE 1588-2008, the responder should check its own twoStepFlag,
             * not the requester's flag. The responder decides whether to send Follow Up
             * based on its own configuration. */
            if ((time->seconds != 0) && (ptpClock->defaultDS.twoStepFlag)) /* not loopback mode */
            {
                /* TWO_STEP responder (mode-c): send Follow_Up carrying responseOriginTimestamp (t3).
                 * We do NOT inject local residence time into correctionField here.
                 * msgPackPDelayRespFollowUp() will propagate the received Pdelay_Req
                 * header.correctionField, allowing the requestor to account for any
                 * correction accumulated on the request path (e.g., by a Transparent Clock). */
...
static void handlePDelayResp(PtpClock *ptpClock, TimeInternal *time, Boolean isFromSelf)
{
    ...
    case P2P:
        ...
        case PTP_SLAVE:
            ...
            //            {
            //                addTime(time, time, &rtOpts->outboundLatency);
            //                /* Loopback mode not supported here */
            //                break;
            //            }
            ...
            isCurrentRequest =
                isSamePortIdentity(&ptpClock->portDS.portIdentity, &ptpClock->msgTmp.presp.requestingPortIdentity);
