...
void initData(PtpClock *ptpClock)
{
    ...
    ptpClock->portDS.announceReceiptTimeout =
        (rtOpts->transportType == TRANSPORT_IEEE_802_1AS) ? DEFAULT_ANNOUNCE_RECEIPT_TIMEOUT_8021AS
    ...
    ptpClock->portDS.logMinPdelayReqInterval =
        (rtOpts->transportType == TRANSPORT_IEEE_802_1AS) ? DEFAULT_PDELAYREQ_INTERVAL_8021AS
