...
{
    // header->transportSpecific. majorSdoId: 0x00, minorSdoId: 0x00
    Nibble transportSpecific = DEFAULT_TRANSPORT_SPECIFIC_1588;
    if (ptpClock != NULL && ptpClock->rtOpts != NULL && ptpClock->rtOpts->transportType == TRANSPORT_IEEE_802_1AS)
    {
        transportSpecific = DEFAULT_TRANSPORT_SPECIFIC_8021AS;
    }
    *(UInteger8 *)(buf + 0) = (UInteger8)((transportSpecific & 0x0F) << 4);
