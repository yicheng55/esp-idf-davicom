...
#if (CURRENT_NETWORK_DRIVER == NETWORK_DRIVER_MODE_DM9058)
  #include "../../../../../utilities/dm9058_edriver_v1.6.1a/dm9058_edriver_extend/dm9058_ptp.h"
#endif
                  ...
                  const TimeInternal *preciseOriginTimestamp, const TimeInternal *correctionField)
{
   ...
   /* compute offsetFromMaster */
    ...
    switch (ptpClock->portDS.delayMechanism)
            {
        case E2E:
            DBG_PRINTF("Path delay (E2E): %ds %09dns\n",
        ...
        default:
            DBG_PRINTF("Final offset (no path delay): %ds %+010dns\n",
                       ptpClock->currentDS.offsetFromMaster.seconds, ptpClock->currentDS.offsetFromMaster.nanoseconds);
            break;
        }
                 ...
                 const TimeInternal *recieveTimestamp, const TimeInternal *correctionField)
{
    ...
    /* 添加調試信息：顯示 T3 和 T4 時間戳 */
    DBG_PRINTF("updateDelay: T3 (DELAY_REQ TX) = %ds %09dns\n",
               delayEventEgressTimestamp->seconds, delayEventEgressTimestamp->nanoseconds);
    DBG_PRINTF("updateDelay: T4 (DELAY_RESP RX) = %ds %09dns\n",
               recieveTimestamp->seconds, recieveTimestamp->nanoseconds);
    DBG_PRINTF("updateDelay: Correction Field = %ds %09dns\n",
               correctionField->seconds, correctionField->nanoseconds);

    /* 計算 Tsm = T4 - T3 */
    subTime(&ptpClock->Tsm, recieveTimestamp, delayEventEgressTimestamp);

    /* 驗證時間戳有效性：檢查 T4 - T3 是否合理 */
    tsm_check = ptpClock->Tsm;

    /* 如果 Tsm < 0 或者 Tsm 的絕對值過大（>100ms），則認為時間戳無效 */
    if (tsm_check.seconds < 0 ||
        (tsm_check.seconds == 0 && tsm_check.nanoseconds < -1000000) ||  /* < -1ms */
        tsm_check.seconds > 0 ||
        (tsm_check.seconds == 0 && tsm_check.nanoseconds > 100000000))   /* > 100ms */
    {
        ERROR("updateDelay: Invalid timestamp detected!\n");
        ERROR("  T3 (TX DELAY_REQ) = %ds %09dns\n",
              delayEventEgressTimestamp->seconds, delayEventEgressTimestamp->nanoseconds);
        ERROR("  T4 (from DELAY_RESP) = %ds %09dns\n",
              recieveTimestamp->seconds, recieveTimestamp->nanoseconds);
        ERROR("  Tsm (T4-T3) = %ds %+010dns (expected: 0s 0-100ms)\n",
              tsm_check.seconds, tsm_check.nanoseconds);
        ERROR("  Skipping meanPathDelay update to prevent corruption\n");

        TimeInternal T3sm;

        T3sm.seconds = delayEventEgressTimestamp->seconds;
        T3sm.nanoseconds = delayEventEgressTimestamp->nanoseconds;
        subTime(&T3sm, recieveTimestamp, &T3sm);
        ERROR("  Approval.Tsm (T4-T3) = %ds %+010dns (expected: 0s 0-100ms)\n",
              T3sm.seconds, T3sm.nanoseconds);

        struct ptptime_t p_time_stamp;
        impl_dm9058_get_tx_timestamp(&p_time_stamp);

        // T3sm= p_time_stamp;
        T3sm.seconds = p_time_stamp.tv_sec;
        T3sm.nanoseconds = p_time_stamp.tv_nsec;
        subTime(&T3sm, recieveTimestamp, &T3sm);

        ERROR("  .updateDelay: Invalid timestamp detected, Again TX timestamp = %ds %09dns\n", p_time_stamp.tv_sec, p_time_stamp.tv_nsec);
    ...
    DBG_PRINTF("updateDelay: Tsm (T4-T3 before CF) = %ds %+010dns\n",
               ptpClock->Tsm.seconds, ptpClock->Tsm.nanoseconds);

    subTime(&ptpClock->Tsm, &ptpClock->Tsm, correctionField);

    DBG_PRINTF("updateDelay: Tsm (after CF) = %ds %+010dns\n",
               ptpClock->Tsm.seconds, ptpClock->Tsm.nanoseconds);
    DBG_PRINTF("updateDelay: Tms (from SYNC) = %ds %+010dns\n",
               ptpClock->Tms.seconds, ptpClock->Tms.nanoseconds);

    addTime(&ptpClock->currentDS.meanPathDelay, &ptpClock->Tms, &ptpClock->Tsm);
    div2Time(&ptpClock->currentDS.meanPathDelay);

    DBG_PRINTF("updateDelay: meanPathDelay (before filter) = %ds %09dns\n",
...
void updateClock(PtpClock *ptpClock)
{
        ...
        || absOffset > MAX_ADJ_OFFSET_NS )
    {
        ...
        if (!ptpClock->servo.noAdjust)
        {
            if (!ptpClock->servo.noResetClock)
            {
                ...
                /* Debug: Print current time before adjustment */
                DBG_PRINTF("updateClock: Current time: %ds %dns\n", 
                           timeTmp.seconds, timeTmp.nanoseconds);
                DBG_PRINTF("updateClock: Offset from master: %ds %dns\n",
                ...
                /* Debug: Print new time after calculation */
                DBG_PRINTF("updateClock: New time after offset correction: %ds %dns\n",
                           timeTmp.seconds, timeTmp.nanoseconds);
    ...
    {
    case E2E:
        printf("updateClock: offset from master:%10d s %11d ns, drift: %10d ns, path delay: %11d ns\r\n",
            ptpClock->currentDS.offsetFromMaster.seconds,
            ptpClock->currentDS.offsetFromMaster.nanoseconds,
            ptpClock->observedDrift,
            ptpClock->currentDS.meanPathDelay.nanoseconds);
    ...
    case P2P:
        printf("updateClock: offset from master:%10d s %11d ns, drift: %10d ns, peer delay: %11d ns\r\n",
            ptpClock->currentDS.offsetFromMaster.seconds,
            ptpClock->currentDS.offsetFromMaster.nanoseconds,
            ptpClock->observedDrift,
    ...
    default:
        printf("updateClock: offset from master:%10d s %11d ns, drift: %10d ns, delay: N/A\r\n",
            ptpClock->currentDS.offsetFromMaster.seconds,
            ptpClock->currentDS.offsetFromMaster.nanoseconds,
            ptpClock->observedDrift);
        break;
