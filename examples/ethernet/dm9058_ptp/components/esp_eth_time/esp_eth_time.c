 ...
 *
 * DM9058 SPI MAC: maps PTP clock API to ETH_MAC_DM9058_* ioctls (not ETH_MAC_ESP_*).
 */
...
int esp_eth_clock_adjtime(clockid_t clk_id, esp_eth_clock_adj_param_t *adj)
{
    ...
    case CLOCK_PTP_SYSTEM:
        if (adj->mode == ETH_CLK_ADJ_FREQ_SCALE) {
            double fs = adj->freq_scale;
            int32_t ppb = (int32_t)lrint((fs - 1.0) * 1e9);
            if (ppb > 500000) {
                ppb = 500000;
            } else if (ppb < -500000) {
                ppb = -500000;
            }
            esp_err_t ret = esp_eth_ioctl(s_eth_hndl, ETH_MAC_DM9058_CMD_ADJ_PTP_FREQ, &ppb);
            if (ret != ESP_OK) {
...
esp_err_t esp_eth_clock_init(clockid_t clock_id, esp_eth_clock_cfg_t *cfg)
{
    ...
    case CLOCK_PTP_SYSTEM: {
        ...
        if (esp_eth_ioctl(cfg->eth_hndl, ETH_MAC_DM9058_CMD_PTP_ENABLE, &ptp_enable) != ESP_OK) {
            return ESP_FAIL;
        }
