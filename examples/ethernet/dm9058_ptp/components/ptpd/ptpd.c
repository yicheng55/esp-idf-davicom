...
#ifdef ESP_PTP_DM9058_UDP_SLAVE
#ifndef CONFIG_CLOCK_ADJTIME_PERIOD_MS
#define CONFIG_CLOCK_ADJTIME_PERIOD_MS 1000
#endif
#ifndef CONFIG_CLOCK_ADJTIME_SLEWLIMIT_PPM
#define CONFIG_CLOCK_ADJTIME_SLEWLIMIT_PPM 500000
#endif
#ifndef CONFIG_NETUTILS_PTPD_DRIFT_AVERAGE_S
#define CONFIG_NETUTILS_PTPD_DRIFT_AVERAGE_S 600
#endif
#endif /* ESP_PTP_DM9058_UDP_SLAVE */
...
#define for_each_cmsghdr(cmsg, mh) \
  for ((cmsg) = CMSG_FIRSTHDR(mh); (cmsg) != NULL; (cmsg) = CMSG_NXTHDR((mh), (cmsg)))
#endif
#ifndef TIMEVAL_TO_TIMESPEC
#define TIMEVAL_TO_TIMESPEC(tv, ts) \
  do { (ts)->tv_sec = (tv)->tv_sec; (ts)->tv_nsec = (long)(tv)->tv_usec * 1000L; } while (0)
#endif
#ifndef ESP_PTP
...
#ifndef HTONL
#define HTONL(x) htonl((uint32_t)(x))
#endif
#ifndef HTONS
#define HTONS(x) htons((uint16_t)(x))
#endif
...
#if defined(ESP_PTP) && !defined(ESP_PTP_DM9058_UDP_SLAVE)
...
struct ptp_state_s
{
  ...
  int event_socket;
  int info_socket;
  uint8_t rxcmsg[CMSG_SPACE(sizeof(struct timeval))];
#else
  int ptp_socket;
...
static int ptp_net_send(FAR struct ptp_state_s *state, void *ptp_msg, uint16_t ptp_msg_len, struct timespec *ts)
{
  ...
  if (ret < 0)
    {
      ESP_LOGE(TAG, "L2 ptp_net_send write failed: errno=%d len=%u", errno, (unsigned)sizeof(eth_frame));
    }
...
#endif /* ESP_PTP && !ESP_PTP_DM9058_UDP_SLAVE */
...
#ifdef ESP_PTP
#ifdef ESP_PTP_DM9058_UDP_SLAVE
  return ptpd_dm9058_clock_gettime(ts);
#else
...
#ifdef ESP_PTP
#ifdef ESP_PTP_DM9058_UDP_SLAVE
  return ptpd_dm9058_clock_settime(ts);
#else
...
#if !defined(ESP_PTP) || defined(ESP_PTP_DM9058_UDP_SLAVE)
/* Smoothly adjust timestamp (software path) or DM9058 frequency. */

static int ptp_adjtime(FAR struct ptp_state_s *state, int64_t delta_ns)
{
#ifdef ESP_PTP_DM9058_UDP_SLAVE
  UNUSED(state);
  return ptpd_dm9058_clock_adjtime_ns(delta_ns, CONFIG_CLOCK_ADJTIME_PERIOD_MS);
#else
                                ...
                                FAR const char *if_key)
{
  ...
  netif = esp_netif_get_handle_from_ifkey(if_key);
  if (netif == NULL) {
    ptperr("esp_netif not found for if_key %s\n", if_key);
    return ERROR;
  }
  if (esp_netif_get_ip_info(netif, &ip) != ESP_OK) {
    ptperr("esp_netif_get_ip_info failed\n");
    return ERROR;
  }
  if (esp_netif_get_mac(netif, state->intf_hw_addr) != ESP_OK) {
    ptperr("esp_netif_get_mac failed\n");
    return ERROR;
  }

  memset(&state->interface_addr, 0, sizeof(state->interface_addr));
  state->interface_addr.sin_family = AF_INET;
  state->interface_addr.sin_addr.s_addr = ip.ip.addr;

  state->tx_socket = socket(AF_INET, SOCK_DGRAM, IPPROTO_IP);
  state->event_socket = socket(AF_INET, SOCK_DGRAM, IPPROTO_IP);
  state->info_socket = socket(AF_INET, SOCK_DGRAM, IPPROTO_IP);
  if (state->tx_socket < 0 || state->event_socket < 0 || state->info_socket < 0) {
    ptperr("socket() failed\n");
    return ERROR;
  }

  memset(&mreq, 0, sizeof(mreq));
  mreq.imr_multiaddr.s_addr = htonl((uint32_t)PTP_MULTICAST_ADDR);
  mreq.imr_interface.s_addr = ip.ip.addr;

  memset(&bind_addr, 0, sizeof(bind_addr));
  bind_addr.sin_family = AF_INET;
  bind_addr.sin_addr.s_addr = HTONL(PTP_MULTICAST_ADDR);
  bind_addr.sin_port = HTONS(PTP_UDP_PORT_EVENT);
  ret = bind(state->event_socket, (struct sockaddr *)&bind_addr, sizeof(bind_addr));
  if (ret < 0) {
    ptperr("bind event port failed: errno=%d\n", errno);
    return ERROR;
  }
  if (setsockopt(state->event_socket, IPPROTO_IP, IP_ADD_MEMBERSHIP, &mreq, sizeof(mreq)) < 0) {
    ptperr("IP_ADD_MEMBERSHIP (event) failed: errno=%d\n", errno);
    return ERROR;
  }
  bind_addr.sin_port = HTONS(PTP_UDP_PORT_INFO);
  ret = bind(state->info_socket, (struct sockaddr *)&bind_addr, sizeof(bind_addr));
  if (ret < 0) {
    ptperr("bind info port failed: errno=%d\n", errno);
    return ERROR;
  }
  if (setsockopt(state->info_socket, IPPROTO_IP, IP_ADD_MEMBERSHIP, &mreq, sizeof(mreq)) < 0) {
    ptperr("IP_ADD_MEMBERSHIP (info) failed: errno=%d\n", errno);
    return ERROR;
  }

  memset(&bind_addr, 0, sizeof(bind_addr));
  bind_addr.sin_family = AF_INET;
  bind_addr.sin_addr.s_addr = ip.ip.addr;
  bind_addr.sin_port = 0;
  ret = bind(state->tx_socket, (struct sockaddr *)&bind_addr, sizeof(bind_addr));
  if (ret < 0) {
    ptperr("bind tx socket failed: errno=%d\n", errno);
    return ERROR;
  }
                                ...
                                FAR const char *interface)
{
  ...
  if (esp_eth_clock_init(CLOCK_PTP_SYSTEM, &clk_cfg) != ESP_OK) {
    ptperr("esp_eth_clock_init failed\n");
    return ERROR;
  }
...
static int ptp_destroy_state(FAR struct ptp_state_s *state)
{
  ...
  if (state->event_socket >= 0) {
    (void)setsockopt(state->event_socket, IPPROTO_IP, IP_DROP_MEMBERSHIP, &mreq, sizeof(mreq));
    close(state->event_socket);
    state->event_socket = -1;
  }
  if (state->info_socket >= 0) {
    (void)setsockopt(state->info_socket, IPPROTO_IP, IP_DROP_MEMBERSHIP, &mreq, sizeof(mreq));
...
static int ptp_send_delay_req(FAR struct ptp_state_s *state)
{
  ...
  if (ptp_gettime(state, &state->delayreq_time) != 0) {
    ESP_LOGE(TAG, "Delay_Req: PTP gettime failed (clock not ready or HW PTP off)");
    return ERROR;
  }
  ...
  if (ret < 0)
    {
      ptperr("sendto failed: %d", errno);
#ifdef ESP_PTP
      ESP_LOGE(TAG, "Delay_Req: send failed ret=%d errno=%d", ret, errno);
#endif
    }
  else
    {
...
#ifdef ESP_PTP
      ESP_LOGI(TAG, "Delay_Req sent (L2/UDP path ok) seq=%" PRId32 " ret=%d",
               (int32_t)ptp_get_sequence(&req.header), ret);
#endif
    }
  ...
  if (state->selected_source_valid && state->can_send_delayreq)
    {
      ...
      delta_ms = timespec_to_ms(&delta);
      need_ms = (int64_t)state->delayreq_interval * MSEC_PER_SEC;
      ...
      if (delta_ms > need_ms)
        ...
        {
          static uint32_t log_ts;
          uint32_t now = esp_log_timestamp();
          if (now - log_ts > 3000)
            {
              log_ts = now;
              ESP_LOGI(TAG, "Delay_Req: interval gate delta_ms=%lld need_ms=%lld intvl=%ld",
                       (long long)delta_ms, (long long)need_ms, state->delayreq_interval);
            }
        }
#endif
    }
#ifdef ESP_PTP
  else if (!state->selected_source_valid)
    {
      static uint32_t log_ts2;
      uint32_t now = esp_log_timestamp();
      if (now - log_ts2 > 4000)
        {
          log_ts2 = now;
          ESP_LOGI(TAG, "Delay_Req: blocked (no valid GM: check domain/announce/sync timeout)");
        }
    }
  else if (!state->can_send_delayreq)
    {
      static uint32_t log_ts3;
      uint32_t now = esp_log_timestamp();
      if (now - log_ts3 > 4000)
        {
          log_ts3 = now;
          ESP_LOGI(TAG, "Delay_Req: blocked (GM ok, can_send_delayreq=0: need stable PI / Follow_Up path)");
        }
    }
#endif
#endif
                                  ...
                                  FAR struct timespec *local_timestamp)
{
  ...
  /* Allow Delay_Req when offset change between consecutive Sync/Follow_Up cycles is small.
   * Old check used llabs(offset)-llabs(last): with ~100–400 ms offset slewing ~10–20 ms per
   * second, that delta never stayed below ns-scale PATH_DELAY_STABILITY_NS -> cnt never rose. */
  int64_t off_step;
  static int cnt = 0;

  if (state->last_offset_ns == 0 && state->remote_time_ns_prev == 0) {
    off_step = INT64_MAX;
  } else {
    off_step = offset_ns - state->last_offset_ns;
  }

  if (llabs(off_step) < (int64_t)CONFIG_NETUTILS_PTPD_PATH_DELAY_STABILITY_NS) {
    if (cnt <= 1) {
      cnt++;
    }
  } else {
  ...
  if (cnt > 1) {
    ...
    ESP_LOGI(TAG, "Delay_Req allowed: offset step stable cnt=%d step_ns=%lld",
             cnt, (long long)off_step);
  }
...
#endif
#endif
      /* Update GM validity before periodic send so Announce/Sync processed this round are visible. */
