...
## ESP-IDF 本專案對應方式（UDP/IPv4）

在本專案（`examples/ethernet/ptp`）的 `UDP/IPv4` transport 中，`lwIP` 目前沒有 Linux 式 `MSG_ERRQUEUE/SO_TIMESTAMPING`。  
因此實作採用 **app-level socket queue**，語意對齊「每個 socket 取得對應 timestamp」：

1. 送出/接收 PTP UDP 封包後，立即透過 DM9058 MAC 介面讀取硬體 TX/RX timestamp。
2. 依 socket（319 event / 320 general）分流，將 `{msg_type, sequence_id, port, timestamp}` 放入 queue。
3. 封包實際處理時再以 `(msg_type, sequence_id, port)` 配對取回 timestamp，避免「只讀最後一筆」造成覆蓋。
4. L2TAP (IEEE 802.3) 路徑維持原本 IREC timestamp 流程，不受 UDP queue 改動影響。
