...
static const char *TAG = "dm9058.phy";
...
static esp_err_t dm9058_update_link_duplex_speed(phy_dm9058_t *dm9058)
{
    esp_err_t ret = ESP_OK;
    esp_eth_mediator_t *eth = dm9058->phy_802_3.eth;
    uint32_t addr = dm9058->phy_802_3.addr;
    ...
    if (dm9058->phy_802_3.link_status != link) {
        ...
        dm9058->phy_802_3.link_status = link;
    }
    return ESP_OK;
err:
    return ret;
...
static esp_err_t dm9058_get_link(esp_eth_phy_t *phy)
{
    esp_err_t ret = ESP_OK;
    phy_dm9058_t *dm9058 = __containerof(esp_eth_phy_into_phy_802_3(phy), phy_dm9058_t, phy_802_3);
    /* Update information about link, speed, duplex */
    ESP_GOTO_ON_ERROR(dm9058_update_link_duplex_speed(dm9058), err, TAG, "update link duplex speed failed");
    return ESP_OK;
err:
    return ret;
}

static esp_err_t dm9058_reset(esp_eth_phy_t *phy)
{
    esp_err_t ret = ESP_OK;
    phy_dm9058_t *dm9058 = __containerof(esp_eth_phy_into_phy_802_3(phy), phy_dm9058_t, phy_802_3);
    uint32_t addr = dm9058->phy_802_3.addr;
    dm9058->phy_802_3.link_status = ETH_LINK_DOWN;
    esp_eth_mediator_t *eth = dm9058->phy_802_3.eth;
    dscr_reg_t dscr;
    ...
    uint32_t to = 0;
    for (to = 0; to < dm9058->phy_802_3.reset_timeout_ms / 10; to++) {
    ...
    ESP_GOTO_ON_FALSE(to < dm9058->phy_802_3.reset_timeout_ms / 10, ESP_FAIL, err, TAG, "PHY reset timeout");
    return ESP_OK;
err:
    return ret;
}

static esp_err_t dm9058_init(esp_eth_phy_t *phy)
{
    esp_err_t ret = ESP_OK;
    ...
    ESP_LOGI(TAG, "PHY ID: OUI=0x%06"PRIX32", Model=0x%02X", oui, model);

    /* DM9058 uses the same OUI as DM9051 (0x606E) and model 0x0A */
    ESP_GOTO_ON_FALSE(oui == 0x606E && model == 0x0A, ESP_FAIL, err, TAG, "wrong chip ID (expected OUI=0x606E, Model=0x0A)");
...
static esp_err_t dm9058_loopback(esp_eth_phy_t *phy, bool enable)
{
    esp_err_t ret = ESP_OK;
    phy_802_3_t *phy_802_3 = esp_eth_phy_into_phy_802_3(phy);
...
static esp_err_t dm9058_set_speed(esp_eth_phy_t *phy, eth_speed_t speed)
{
    esp_err_t ret = ESP_OK;
    phy_802_3_t *phy_802_3 = esp_eth_phy_into_phy_802_3(phy);
    esp_eth_mediator_t *eth = phy_802_3->eth;
...
esp_eth_phy_t *esp_eth_phy_new_dm9058(const eth_phy_config_t *config)
{
    ...
    ESP_GOTO_ON_FALSE(dm9058, NULL, err, TAG, "calloc dm9058 failed");
    ESP_GOTO_ON_FALSE(esp_eth_phy_802_3_obj_config_init(&dm9058->phy_802_3, config) == ESP_OK,
                      NULL, err, TAG, "configuration initialization of PHY 802.3 failed");

    /* redefine functions which need to be customized for sake of dm9058 */
    dm9058->phy_802_3.parent.init = dm9058_init;
    dm9058->phy_802_3.parent.reset = dm9058_reset;
    dm9058->phy_802_3.parent.get_link = dm9058_get_link;
    dm9058->phy_802_3.parent.loopback = dm9058_loopback;
    dm9058->phy_802_3.parent.set_speed = dm9058_set_speed;

    return &dm9058->phy_802_3.parent;
err:
    if (dm9058 != NULL) {
        free(dm9058);
    }
